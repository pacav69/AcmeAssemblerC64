"""
Sublime Text plugin for Acmeembler development for displaying information
...
class AcmeTooltip:
    ...
    def _register_settings(self):
        ...
        changes.
        """
        self.settings = sublime.load_settings("AcmeTooltips.sublime-settings")
    ...
    def load(self):
        ...
        default_css = os.path.join(plug_path, 'AcmeAssemblerC64/css/default.css')

        AcmeTooltip.css_file = self.settings.get('css_file', default_css)

        AcmeTooltip.scopes = self.settings.get('scopes', [])
                    ...
                    with open(os.path.join(dirname, fname),
                        ...
                        try:
                            ...
                            logging.error('Error in JSON file: %s, %s',
                                          fname, exc.message)
                            continue
                    AcmeTooltip.help_map.update(data)
                except EnvironmentError:
                    logging.error('Cannot access file: %s', fname)

        logging.debug('Help map:\n%s', AcmeTooltip.help_map)


class AcmeTooltipsCommand(sublime_plugin.ViewEventListener):
    """
    ...
    def on_hover(self, point, hover_zone):
        ...
        if AcmeTooltip.scopes[0] not in selection:
            return
        ...
        if text.lower() in AcmeTooltip.help_map:
            help_message = AcmeTooltip.help_map[text.lower()]
            logging.debug('Help message: %s', help_message)
            html_message = TOOLTIP % {'css': AcmeTooltip.css_file,
                                      'title': text,
                                      'name': help_message['name'],
                                      'desc': help_message['descr'],
                                      'url': help_message['url']}
    ...
    def show_numeric_tooltip(self, val, point):
        html_message = NUMBER_TOOLTIP.format(css=AcmeTooltip.css_file,
                                             value=val)
        self.show_tooltip(html_message, point)
...
def plugin_loaded():
    ...
    tooltip = AcmeTooltip()
    tooltip.load()
    logging.info('AcmeTooltips loaded.')
